...
function main()
    ...
    length(responses) == 2 || error("worker smoke expected 2 responses, got $(length(responses))")
    ...
    responses[2].result.mode == "mixed_enabled" || error("worker solve returned unexpected mode")
